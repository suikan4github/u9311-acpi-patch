...
function patch-fedora() {
    echo "[patch-fedora] For Fedora Workstation."
    ...
    echo "[patch-fedora] Creating patch file fujitsu-vdd.patch..."
    ...
    echo "[patch-fedora] Creating temporary container ${CONTAINER}..."
    ...
    echo "[patch-fedora] Installing acpica-tools and patch inside container ${CONTAINER}..."
    ...
    echo "[patch-fedora] Disassembling the ACPI table inside container ${CONTAINER}..."
    toolbox run -c ${CONTAINER} -- iasl -d SSDT4.aml

    # Apply the patch to the disassembled ACPI table, inside container.
    echo "[patch-fedora] Applying the patch inside container ${CONTAINER}..."
    toolbox run -c ${CONTAINER} -- patch < fujitsu-vdd.patch

    # If patch fail, exit the shell function.
    if [ $? -ne 0 ]; then
        echo "[patch-fedora] Failed to apply the patch. Remove the temporaly container and exit."
    ...
    echo "[patch-fedora] Reassembling the patched ACPI table inside container ${CONTAINER}..."
    toolbox run -c ${CONTAINER} -- iasl -sa SSDT4.dsl

    # Remove the temporary container after patching.
    echo "[patch-fedora] Removing temporary container ${CONTAINER}..."
    toolbox rm ${CONTAINER} -f
    ...
    if [ -e /run/ostree-booted ]; then
        ...
        echo "[patch-fedora] Detected Fedora Atomic Desktop. Copying the patched ACPI table to ${DST}..."
    else
        # Fedora Workstation / Spin 用の処理
        DST=/usr/local/etc/acpi-overrides
        echo "[patch-fedora] Detected Fedora Workstation / Spin. Copying the patched ACPI table to ${DST}..."
    fi
    echo "[patch-fedora] Copying the patched ACPI table to ${DST}..."
    sudo mkdir -p ${DST}
sudo cp ${SRC}/SSDT4.aml ${DST}
    ...
    echo "[patch-fedora] Creating dracut configuration file /etc/dracut.conf.d/99-acpi-override.conf..."
...
EOF
    ...
    if [ -e /run/ostree-booted ]; then
        echo "[patch-fedora] Detected Fedora Atomic Desktop. Regenerating the initramfs using rpm-ostree..."
        # Fedora Atomic Desktop (Kinoite, Silverblue, etc.) 用の処理
        sudo rpm-ostree initramfs --enable

        # In the case of error, re-enable the initramfs to regenerate it.
        if [ $? -ne 0 ]; then
            echo "[patch-fedora] Failed to generate the initramfs. Re-enabling it."
        ...
        # Fedora Workstation / Spin 用の処理
        echo "[patch-fedora] Detected Fedora Workstation / Spin. Regenerating the initramfs using dracut..."
        sudo dracut --force --verbose
    fi

    echo "[patch-fedora] ACPI patch applied successfully. Please reboot your system for the changes to take effect."
    return 0
}